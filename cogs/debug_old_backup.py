...
def _is_allowed_user(interaction: discord.Interaction) -> bool:
    """
    Restringe el uso del debug SOLO al owner del bot.
    
    CRÍTICO DE SEGURIDAD: Solo usuarios en DEBUG_ALLOWLIST pueden ejecutar comandos debug.
    NO se permite a admins de servidores, SOLO al owner del bot.
    """
    if interaction.user is None:
        return False
    
    # SOLO permitir usuarios en DEBUG_ALLOWLIST (owner del bot)
    allow = os.getenv('DEBUG_ALLOWLIST', '')
    if not allow:
        # Si no hay allowlist, NADIE puede usar debug (seguro por defecto)
        return False
